...
| **插件删除时组件残留** | 删除/卸载插件后，插件创建的概览页按钮仍然存在，需重启程序才消失 | 删除插件时自动调用 `onUninstall` 清理 UI 组件 |
| **插件重启后不显示** | 程序升级或重启后，自动启动内核时概览页没有面板按钮 | 使用 `initWatcher` 监听内核状态，替代一次性事件，确保始终与内核运行状态同步 |
| **插件配置切换不生效** | 切换面板配置后，需重启内核才生效 | 在配置变更时强制刷新 UI 组件，并更新监听器的闭包上下文 |

> **⚠️ 注意**：如果您之前安装过旧版面板插件（如 `v1.0.1` 及以下），更新后可能会在概览页出现重复的旧面板按钮。这是因为旧版插件没有卸载清理逻辑。**完全退出并重启 GFC Plus 程序**即可清除这些残留按钮。
...
- **修复版插件**：包含 `Clash API 仪表板 (修复版)`，配置更改后立即生效，无需重启内核
