...
func extractEmbeddedFiles(fs embed.FS) {
	...
	if err := os.MkdirAll(resolvePath(pluginDst), os.ModePerm); err != nil {
		log.Printf("Failed to create plugin directory: %v", err)
	}
	if err := os.MkdirAll(resolvePath(thirdPartyDst), os.ModePerm); err != nil {
		log.Printf("Failed to create third-party directory: %v", err)
	}
